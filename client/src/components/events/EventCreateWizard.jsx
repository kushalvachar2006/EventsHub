...
const EventCreateWizard = ({ onSubmit, initialValues = null }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if ((initMode || "").toLowerCase() === "online")
        setMeetingLink(initialValues.location || "");
      ...
      if ((initialValues.category || initialValues.eventType) === "Hackathon") {
        ...
        if (dur === "24") {
          if (initialValues.date)
            setStartDateTime24(new Date(initialValues.date));
          if (initialValues.endDate)
            setEndDateTime24(new Date(initialValues.endDate));
        }
      }
      // Team size and prize
      if (typeof initialValues.minTeamSize === "number")
        setMinTeamSize(String(initialValues.minTeamSize));
      if (typeof initialValues.teamSize === "number")
        setMaxTeamSize(String(initialValues.teamSize));
      ...
      if (initialValues.organizerWebsite || initialValues.website)
        setOrganizerWebsite(
          initialValues.organizerWebsite || initialValues.website
        );
    } catch {
    ...
    if (step === 2) {
      if (category === "Hackathon" && hackDuration === "24") {
        return (
          !!startDateTime24 &&
          !!endDateTime24 &&
          new Date(endDateTime24) > new Date(startDateTime24)
        );
      }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const endISO = (() => {
        if (category === "Hackathon" && hackDuration === "24") {
          return endDateTime24 ? new Date(endDateTime24).toISOString() : null;
        }
      ...
      const regISO = regDeadline ? new Date(regDeadline).toISOString() : null;

      // Build FormData with only changed fields in edit mode; all fields in create mode
      const form = new FormData();
      const appendIfChanged = (key, newVal, oldVal) => {
        const a = newVal == null ? "" : String(newVal);
        const b = oldVal == null ? "" : String(oldVal);
        if (a !== b) form.append(key, newVal);
      };

      if (initialValues) {
        const init = initialValues || {};
        // Basics
        appendIfChanged("title", title, init.title);
        appendIfChanged(
          "description",
          description || "",
          init.description || ""
        ...
        appendIfChanged(
          "date",
          dateTimeISO || "",
          init.date ? new Date(init.date).toISOString() : ""
        );
        appendIfChanged(
          "endDate",
          endISO || "",
          init.endDate ? new Date(init.endDate).toISOString() : ""
        );
        appendIfChanged(
          "registrationDeadline",
          regISO || "",
          init.registrationDeadline
            ? new Date(init.registrationDeadline).toISOString()
            : ""
        );
        // Category / type
        appendIfChanged(
          "category",
          category || "",
          init.category || init.eventType || ""
        );
        if (category === "Hackathon") {
          appendIfChanged(
            "eventType",
            "Hackathon",
            init.eventType || init.category || ""
          );
          appendIfChanged(
            "hackathonDuration",
            hackDuration || "",
            String(init.hackathonDuration || "")
          );
        }
        // Location
        const currentLocation = mode === "Online" ? meetingLink : venue;
        appendIfChanged("location", currentLocation || "", init.location || "");
        appendIfChanged("eventMode", mode, init.eventMode || init.mode || "");
        // Organizer
        appendIfChanged(
          "organizerName",
          organizerName || "",
        ...
        appendIfChanged(
          "organizerWebsite",
          organizerWebsite || "",
          init.organizerWebsite || init.website || ""
        );
        // Team size and prize
        appendIfChanged(
          "minTeamSize",
        ...
        if (regISO) form.append("registrationDeadline", regISO);
        ...
        if (prizePool !== "" && prizeCurrency) {
          form.append("prizeCurrency", prizeCurrency);
        }
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-panel p-6 md:p-8 space-y-6">
      ...
      <div className="flex items-center justify-center gap-3 text-xs text-slate-400">
        ...
        >
          <FileText className="h-3 w-3" /> Basics
        </span>
        <span>•</span>
        <span className={`${step === 2 ? "text-white" : "text-slate-400"}`}>
          <CalendarIcon className="inline h-3 w-3" /> Schedule
        </span>
        <span>•</span>
        <span className={`${step === 3 ? "text-white" : "text-slate-400"}`}>
          <ImageIcon className="inline h-3 w-3" /> Location & Media
        </span>
        <span>•</span>
        <span className={`${step === 4 ? "text-white" : "text-slate-400"}`}>
          <CheckCircle2 className="inline h-3 w-3" /> Organizer
        </span>
      </div>
          ...
          <div>
            ...
            <div className="rounded-xl border border-white/10 overflow-hidden bg-slate-900/40">
              <div className="flex items-center gap-2 px-3 py-2 border-b border-white/10 bg-slate-900/30 text-xs">
                ...
                <button
                  type="button"
                  className="px-2 py-1 rounded hover:bg-white/5 underline"
                  onMouseDown={(e) => {
                    e.preventDefault();
                    saveSelection();
                    exec("underline");
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium mb-2">
                  End time{" "}
                  {hackDuration
                    ? `(auto ${hackDuration}h from start, editable)`
                    : ""}
                </label>
                <DatePicker
                  selected={endTime}
          ...
          {category === "Hackathon" && hackDuration === "24" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <DatePicker
                  selected={startDateTime24}
                  onChange={setStartDateTime24}
                  showTimeSelect
                  timeIntervals={15}
              ...
              <div>
                ...
                <DatePicker
                  selected={endDateTime24}
                  onChange={setEndDateTime24}
                  showTimeSelect
                  timeIntervals={15}
                  dateFormat="MMM d, yyyy h:mm aa"
                  placeholderText="Select"
      ...
      {step === 3 && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">
                {mode === "Online" ? "Meeting link" : "Venue address"}
              </label>
              <input
                className="input-field"
                value={mode === "Online" ? meetingLink : venue}
                onChange={(e) =>
                  mode === "Online"
                ...
                placeholder={
                  mode === "Online" ? "https://..." : "e.g., Main Auditorium"
                }
              />
            </div>
          </div>
        ...
        <div className="space-y-5">
          ...
          {(category === "Competition" || category === "Hackathon") && (
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1">
                Optional, shown to participants for more info.
              </p>
            </div>
          )}
          ...
          <div className="rounded-xl border border-white/10 p-4 bg-slate-900/40">
            <div className="text-sm text-slate-300">
              ...
              <div>
                <span className="text-slate-400">Title:</span> {title}
              </div>
              <div>
                <span className="text-slate-400">Type:</span> {category || "—"}{" "}
                {category === "Hackathon" && hackDuration
                  ? `(${hackDuration}h)`
                  : ""}
              </div>
              <div>
                ...
                {category === "Hackathon" && hackDuration === "24"
                  ? (startDateTime24
                      ? new Date(startDateTime24).toLocaleString()
                      : "—") +
                    " → " +
                    (endDateTime24
                      ? new Date(endDateTime24).toLocaleString()
                      : "—")
                  : eventDate
              ...
              <div>
                ...
                {regDeadline ? new Date(regDeadline).toLocaleString() : "—"}
              </div>
              <div>
                <span className="text-slate-400">Location:</span>{" "}
                {mode === "Online" ? meetingLink : venue}
              </div>
              <div>
                <span className="text-slate-400">Team size:</span>{" "}
                {minTeamSize || "—"}
                {(minTeamSize || maxTeamSize) && " to "} {maxTeamSize || "—"}
              </div>
              <div>
              ...
              {(category === "Competition" || category === "Hackathon") && (
                <div>
                  <span className="text-slate-400">Organizer website:</span>{" "}
                  {organizerWebsite || "—"}
                </div>
              )}
            </div>
      ...
      <div className="flex items-center justify-between gap-3 pt-2">
        <button
          ...
          onClick={prev}
          disabled={step === 1}
          className="px-4 py-2 rounded-lg bg-white/5 text-slate-200 disabled:opacity-50 flex items-center gap-2"
        >
